...
#
#Example:
#   runMode_Grid.sh GE11-VII-L-CERN-0001 config/configAnalysis.cfg config/Mapping_GE11-VII-L.cfg 1nh /path/to/my/data/directory
...
COUNTER=0
for file in $(ls $CMS_GEM_EOS_COMM/$1/*dataTree.root )
do
    #increment counter
    ...
    #Replace filenames - Analysis
    for substr in $(echo $FILE_ANA | tr "/" "\n")
    do
        if strcmp $substr ".cfg"
        then
            sed -i -- "s@CONFIGFILE_ANALYSIS@$substr@g" $FILE_RUN
            break
        fi
    done
    ...
    do
        if strcmp $substr "dataTree.root"
        then
    ...
    echo "cp $FILE_ANA $DIR_JOBNUM" >> $FILE_SCRIPT
    ...
    echo "fi" >> $FILE_SCRIPT
    echo "mv *Ana.root $DIR_OUTPUT/$NAME_DET" >> $FILE_SCRIPT
    echo "mv *Ana*.txt $DIR_OUTPUT/$NAME_DET" >> $FILE_SCRIPT
    echo "" >> $FILE_SCRIPT
